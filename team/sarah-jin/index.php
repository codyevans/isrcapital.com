<?php $title = "Sarah Jin"; ?>
<?php include "../../template/_header.php"; ?>

<div class="herowidth">
			<img src="../../images/header/hero3.jpg" alt="ISRCapital Team Photo in front of our office.">
			<img src="../../images/header/hero3.jpg" alt="ISRCapital Team Photo in front of our office.">
		</div>
	</div>

<!--body content-->
	<div class="container">

		<div class="mission bioSpace">
			<h2>our team</h2>
		</div>

		<div class="teamModule">
			<div class="teamTitles">
				<h3 class="name">Sarah Jin</h3>
				<h4 class="title">Marketing Assistant</h4>
			</div>

			<div class="teamContent">
				<p>
					Sarah Jin is an investor relations specialist at ISRC. She originally came from Shanghai, China. She received her Bachelor’s degree at Shanghai International Studies University and is earning her M.B.A. degree at Boise State University. Sarah is fluent in both English and Chinese, as well as communicative Japanese. During the time at ISRC, Sarah is helping with both Phase I and Phase II investors, providing consultation for the project analysis and progress, assisting application work between agents and investors. Sarah has her business background in both China and United States, which helps her understand the investors’ needs and expectations.
				</p>
			</div>
		</div><?php //end module ?>

		<div class="teamGroup">

			<a href="/team/raymond-ku/" class="teamBox" title="click for more...">
				<h2><span>Raymond Ku</span><br>Chairman</h2>
				<p>Click to read Raymond's bio...</p>
			</a>

			<a href="/team/sima-muroff/" class="teamBox" title="click for more...">
				<h2><span>Sima Muroff</span><br>Chief Executive Officer</h2>
				<p>Click to read Sima's bio...</p>
			</a>

			<a href="/team/debra-riddle/" class="teamBox" title="click for more...">
				<h2><span>Debra Riddle</span><br> Chief Financial Officer</h2>
				<p>Click to read Debra's bio...</p>
			</a>

			<a href="/team/fred-payne/" class="teamBox" title="click for more...">
				<h2><span>Fred Payne</span><br>Chief Accounting Officer, CPA</h2>
				<p>Click to read Fred's bio...</p>
			</a>

			<a href="/team/ryan-mcdermott/" class="teamBox" title="click for more...">
				<h2><span>Ryan McDermott</span><br>Vice President & Senior Geologist</h2>
				<p>Click to read Ryan's bio...</p>
			</a>

			<a href="/team/kirk-williams/" class="teamBox" title="click for more...">
				<h2><span>W. Kirk Williams</span><br>Corporate Counsel / Holdings Manager</h2>
				<p>Click to read Kirks's bio...</p>
			</a>

			<a href="/team/mark-axtell/" class="teamBox" title="click for more...">
				<h2><span>Mark E Axtell</span><br>Vice President, Investment Banking</h2>
				<p>Click to read Mark's bio...</p>
			</a>

			<a href="/team/tina-smith/" class="teamBox" title="click for more...">
				<h2><span>Tina J. Smith</span><br>Controller</h2>
				<p>Click to read Tina's bio...</p>
			</a>

			<a href="/team/dhwanit-pambhar/" class="teamBox" title="click for more...">
				<h2><span>Dhwanit Pambhar</span><br>Economist</h2>
				<p>Click to read Dhwanit's bio...</p>
			</a>

			<a href="/team/kayte-grigsby/" class="teamBox" title="click for more...">
				<h2><span>Kayte Grigsby</span><br>Project Manager</h2>
				<p>Click to read Kayte's bio...</p>
			</a>

			<a href="/team/cody-evans/" class="teamBox" title="click for more...">
				<h2><span>Cody Evans</span><br>Graphic Designer</h2>
				<p>Click to read Cody's bio...</p>
			</a>

			<a href="/team/brian-jackson/" class="teamBox" title="click for more...">
				<h2><span>Brian Jackson</span><br>Project Engineering Manager</h2>
				<p>Click to read Brian's bio...</p>
			</a>
				
			<a href="/team/adam-tillquist/" class="teamBox" title="click for more...">
				<h2><span>Adam Tillquist</span><br>Geologist</h2>
				<p>Click to read Adam's bio...</p>
			</a>

			<a href="/team/bruce-thorndycraft/" class="teamBox" title="click for more...">
				<h2><span>Bruce Thorndycraft</span><br>Metallurgical Engineer</h2>
				<p>Click to read Bruce's bio...</p>
			</a>

			<a href="/team/jessica-ader/" class="teamBox" title="click for more...">
				<h2><span>Jessica Ader</span><br>Geologist / GIS Specialist</h2>
				<p>Click to read Jessica's bio...</p>
			</a>

			<a href="/team/sanja-bell/" class="teamBox" title="click for more...">
				<h2><span>Sanja Bell</span><br>Executive Assistant</h2>
				<p>Click to read Sanja's bio...</p>
			</a>

			<a href="/team/sarah-jin/" class="teamBox teamBox-active" title="click for more...">
				<h2><span>Sarah Jin</span><br>Marketing Assistant</h2>
				<p>Click to read Sarah's bio...</p>
			</a>

		</div> <?php //end team group ?>	

	</div><?php //end container ?>
	
<?php include "../../template/_footer.php"; ?>